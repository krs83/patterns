"""
Фасад — структурный паттерн, который предоставляет простой интерфейс к сложной системе классов,
библиотеке или фреймворку.

Суть:
Скрывает сложную внутреннюю логику

Предоставляет удобный "пульт управления" для часто используемых операций

Пример из жизни:
Как кнопка "Пуск" в автомобиле — вы нажимаете одну кнопку, а под капотом запускается целая цепочка
сложных процессов (проверка систем, подача топлива, запуск стартера и т.д.).
"""
